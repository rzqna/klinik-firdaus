...
@section('content')
    <main id="content" class="p-6 pt-20 lg:pt-6 lg:ml-64 min-h-screen">
        <div class="bg-white p-8 rounded-lg shadow-md">
            ...
            {{-- Form untuk mengedit pekerjaan --}}
            {{-- Action form ini akan mengarah ke method 'update' di JabatanController. --}}
            {{-- $pekerjaan->id akan digunakan untuk mengidentifikasi data yang akan diupdate. --}}
            <form action="{{ route('pekerjaan.update', $pekerjaan->id) }}" method="POST">
                ...
                <div class="space-y-6">
                    ...
                    <div class="mb-4">
                        <label for="pekerjaan" class="block text-sm font-medium text-gray-700 mb-2">Nama Jabatan:</label>
                        <input
                            type="text"
                            id="pekerjaan"
                            name="pekerjaan"
                            class="shadow-sm appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-blue-500 focus:border-blue-500 transition duration-150 ease-in-out @error('pekerjaan') border-red-500 @enderror"
                            value="{{ old('pekerjaan', $pekerjaan->pekerjaan) }}" {{-- Mengisi nilai awal dari database --}}
                    ...
                    <div class="flex items-center justify-between mt-8 pt-4 border-t border-gray-200">
                        <a href="{{ route('pekerjaan.index') }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100 transition duration-150 ease-in-out">
                            Batal
                        </a>
