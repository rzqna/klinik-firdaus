    ...
    <main id="content" class="p-6 pt-20 lg:pt-6 lg:ml-64 min-h-screen">
        <div class="bg-white p-8 rounded-lg shadow-md">
            ...
            <div class="flex justify-end mb-4">
                <a href="{{ route('jabatan.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-300 ease-in-out">
                    + Tambah Data
                </a>
            </div>
                    ...
                    <tbody>
                        @forelse ($jabatans as $jabatan)
                            <tr class="bg-white border-b border-gray-100 hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">{{ $jabatan->jabatan }}</td>
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    {{-- Tombol Edit (Icon Pensil) --}}
                                    <a href="{{ route('jabatan.edit', $jabatan->id) }}" class="inline-flex items-center p-2 text-yellow-600 hover:text-yellow-800 transition duration-150 ease-in-out" title="Edit">
                                    ...
                                    <form action="{{ route('jabatan.destroy', $jabatan->id) }}" method="POST" class="inline-block ml-1" onsubmit="return confirm('Apakah Anda yakin ingin menghapus jabatan ini?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-4 text-center text-gray-500">Belum ada data jabatan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
